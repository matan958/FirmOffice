...
export default function DocumentViewerPage() {
  ...
  const isPdf = document !== null && document !== 'missing' && document.file.contentType === 'application/pdf';
  ...
  if (document === 'missing') {
    return (
      <main className="mx-auto max-w-2xl p-8">
        ...
        <Link to="/inbox" className="mt-4 inline-block text-sm text-brand-600 underline">
          ← Back to inbox
        </Link>
      </main>
    );
  ...
  if (document === null) {
    return <main className="p-8 text-sm text-ink-600">Loading…</main>;
  }
  ...
  return (
    <main className="flex h-dvh flex-col">
      {/* ── header ── */}
      <header className="shrink-0 border-b border-ink-200 px-6 py-3">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
          <Link to="/inbox" className="text-sm text-brand-600 underline-offset-2 hover:underline">
            ← Inbox
          </Link>
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <section className="min-h-0 flex-1 overflow-auto bg-ink-100 p-4">
          {preview.status === 'loading' && <p className="text-sm text-ink-600">Loading preview…</p>}
          ...
          {preview.status === 'ready' && isPdf && (
            <div className="flex flex-col items-center gap-3">
              <PdfDocument
                ...
                loading={<p className="text-sm text-ink-600">Rendering…</p>}
              >
                <PdfPage
              ...
              {numPages > 1 && (
                <div className="flex items-center gap-3 text-sm">
                  <button
                    ...
                    className="rounded-md border border-ink-200 bg-white px-2 py-1 disabled:opacity-40"
                  >
                    ‹ Prev
                  </button>
          ...
          {preview.status === 'ready' && !isPdf && (
            document.file.contentType.startsWith('image/') ? (
              ...
              <div className="mx-auto max-w-md rounded-md border border-ink-200 bg-white p-6 text-center">
        ...
        <section className="min-h-0 flex-1 overflow-auto border-t border-ink-200 p-4 lg:border-l lg:border-t-0">
          ...
          {document.ocr && pageText.trim().length === 0 && (
            <p className="text-sm text-amber-700">
              No text detected on this page. If the original is a scan, it may be blank,
              too faint, or upside down.
            </p>
          )}
